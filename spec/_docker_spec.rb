...
describe 'drone::_docker' do
  ...
  context 'When attributes are overridden, on ubuntu' do
    cached(:chef_run) do
      runner = ChefSpec::ServerRunner.new(platform: 'ubuntu', version: '16.04') do |node, _server|
        ...
        node.override['drone']['docker']['bridge_ip'] = '192.168.1.1/24'
    ...
    it 'sets docker bridge ip to 192.168.1.1/24' do
      expect(chef_run).to start_docker_service_manager('default').with(bip: '192.168.1.1/24')
    end
  ...
  context 'When attributes are overridden, on centos' do
    cached(:chef_run) do
